...
async def scrape_website(project_id: str, payload: ScrapeRequest, user_id: str = Depends(get_current_user)):
  ...
  # 1. Clean and force the protocol if the user forgot it
    target_url = payload.url.strip()
    if not target_url.startswith(("http://", "https://")):
        target_url = "https://" + target_url

    # 2. Use an aggressive browser User-Agent to dodge bot-blockers
    req_headers = {
    ...
    try:
        # Changed payload.url to target_url
        response = http_requests.get(target_url, headers=req_headers, timeout=15, allow_redirects=True)
        response.raise_for_status()
    except Exception as e:
        # This will print the actual technical reason in your console logs
        print(f"[SCRAPE CRASH] URL: {target_url} | Error: {str(e)}")
        raise HTTPException(status_code=400, detail=f"Scraper blocked or invalid URL: {str(e)}")
